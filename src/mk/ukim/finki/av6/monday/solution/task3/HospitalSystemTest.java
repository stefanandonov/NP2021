package mk.ukim.finki.av6.monday.solution.task3;
...
class HospitalSystem<T extends Patient & Comparable<T>> {
    ...
    public void printResults() {
        ...
        /**
         * You only need one findAllWithDoctor method, here we show the two ways to implement this.
         */
        ...
        PatientService<T> findAllWithDoctorWithoutStreams = (patients, filter) -> {
            ArrayList<T> tmp = new ArrayList<>();
            for (T p : patients) {
                if (p.getDoctor().equals(filter))
                    tmp.add(p);
            }
            return tmp;
        };
...
class PatientFromDB1 implements Patient, Comparable<PatientFromDB1> {
    ...
    /**
     * It is important to notice that this method is implemented as we need it!
     */
    @Override
